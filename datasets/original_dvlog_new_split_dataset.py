...
class OriginalDVlogNewSplitDataset(AcumenDataset):
    def __init__(self, args, kind = 'train', data_transforms = None):
        ...
        self.df = pd.read_csv(f'{args.environment["d-vlog"]}/splits/original/d-vlog.csv')
        self.df['duration'] = self.df['duration'].apply(lambda x: math.floor(float(x.replace(',', '.'))))

        if kind in ['train']:
            not_missed_video_ids = pd.read_csv(f'{args.environment["d-vlog"]}/splits/training.csv', index_col = 0)['video_id'].tolist()

        if kind in ['validation']:
            not_missed_video_ids = pd.read_csv(f'{args.environment["d-vlog"]}/splits/validation.csv', index_col = 0)['video_id'].tolist()

        if kind in ['test']:
            not_missed_video_ids = pd.read_csv(f'{args.environment["d-vlog"]}/splits/test.csv', index_col = 0)['video_id'].tolist()
