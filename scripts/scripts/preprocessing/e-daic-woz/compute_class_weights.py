...
def compute_class_weights():
    dir = '/home/banamar/perceiving-depression/data/databases/E-DAIC-WOZ/'
    df = pd.read_csv(f'{dir}/splits/training.csv')
    print(df)

    frames_depression = df[df['PHQ_Binary'] == 1]['duration'].sum()
    frames_no_depression = df[df['PHQ_Binary'] == 0]['duration'].sum()
