...
CONFIG_FILE="configs/train_configs/baseline_original_dvlog_config.yaml"
GROUP=baseline-original-dvlog-modality-ablation

# AV
python main.py --run_id 1 --save_model 1 --presence_threshold $PRESENCE_THRESHOLD --name av-run-1 --seconds_per_window $SECONDS_PER_WINDOW --group $GROUP --mode $WANDB_MODE --trainer $TRAINER --scheduler $SCHEDULER --scheduler_args.max_lr $MAX_LR --scheduler_args.end_epoch $EPOCHS --n_temporal_windows $N_TEMPORAL_WINDOWS --epochs $EPOCHS --batch_size $BATCH_SIZE --config_file $CONFIG_FILE --env $ENV $MODEL_ARGS --dataset original-d-vlog --use_modalities $AV
