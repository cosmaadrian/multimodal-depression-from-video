...
# Audio-visual + eyes
python main.py --run_id 1 --save_model 1 --presence_threshold $PRESENCE_THRESHOLD --name av-eyes-run-1 --seconds_per_window $SECONDS_PER_WINDOW --group $GROUP --mode $WANDB_MODE --trainer $TRAINER --scheduler $SCHEDULER --scheduler_args.max_lr $MAX_LR --scheduler_args.end_epoch $EPOCHS --n_temporal_windows $N_TEMPORAL_WINDOWS --epochs $EPOCHS --batch_size $BATCH_SIZE --config_file configs/baseline_config.yaml --env $ENV $MODEL_ARGS --use_modalities $AUDIO_VISUAL $EYES
